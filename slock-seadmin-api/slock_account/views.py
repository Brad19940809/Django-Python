...
from slock_account.serializers import *
...
class Register(LoggedAPIView):
    ...
    def post(self, request):
        ...
        if pp.is_valid():
            ...
            account.save()
            password_history = PasswordHistory(account=account, t=timezone.now(), ip=get_ip(request), password=password)
    ...
    def get(self, request):
        ...
        if pp.is_valid():
            ...
            if user is not None:
                ...
                access_log = AccessLog(account=account, t=timezone.now(), ip=get_ip(request), token=token)
                access_log.save()
    ...
    def put(self, request):
        ...
        if pp.is_valid():
            ...
            if user is not None:
                ...
                account.save()
                password_history = PasswordHistory(account=account, t=timezone.now(), ip=get_ip(request), password=password_new)
                password_history.save()
...
class Misc(LoggedAPIView):
    ...
    def post(self, request):
        ...
        if pp.is_valid():
            ...
            field = pp.validated_data['field']
            value = pp.validated_data['value']
            misc = dict()
            if account.misc is not None:
